...
async def biorxiv_search(query: str, max_results: int = 10):
    """
        ...
        while tries < max_retries:
            try:
                ...
                if tries == max_retries:
                    print(f"Failed to connect to bioRxiv API after {max_retries} attempts: {e}")
                    break
                print(f"Biorxiv search attempt {tries} failed, retrying...")
        else:
            continue
        break
...
if __name__ == '__main__':
    paper = biorxiv_search(query="THBS2 extracellular matrix protein cell-cell interactions cell-matrix interactions", max_results=3)
    print(paper)
